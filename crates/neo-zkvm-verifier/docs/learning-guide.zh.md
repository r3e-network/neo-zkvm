# neo-zkvm-verifier 技术学习指南

这份指南把 `neo-zkvm-verifier` 当作 Neo N4 的一个技术单元来解释。它不是源码阅读图，而是帮助读者理解：这个单元负责什么、哪些技术假设保证它正确、数据如何移动、状态如何变化、证据如何被验证、它如何接入 Neo N4 的整体架构。
...
| 输入 | 证明封装 <br> 验证键 <br> 期望公开输出 |
| 职责 | 检查证明格式 <br> 验证公开值 <br> 报告有效性 |
| 输出 | 验证结果 <br> 错误原因 <br> 审计证据 |
...
`neo-zkvm-verifier` 接收 证明封装 | 验证键 | 期望公开输出，拥有的边界是：检查证明格式 | 验证公开值 | 报告有效性。它输出 验证结果 | 错误原因 | 审计证据，然后由 zkVM 用户 | L2 prover 服务 | L1 验证适配器 消费。
