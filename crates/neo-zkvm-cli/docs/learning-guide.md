# neo-zkvm-cli Technical Learning Guide

This guide explains `neo-zkvm-cli` as a Neo N4 technical unit. It is written for architecture learning: what the unit is responsible for, which assumptions make it correct, how data moves, how state changes, how evidence is checked, and where it plugs into the wider Neo N4 stack.
...
| Inputs | CLI command <br> script/proof files <br> prover options |
| Responsibilities | Parse commands <br> Use shared opcode metadata <br> Run prove/verify workflows |
| Outputs | assembled script <br> proof report <br> inspection output |
...
`neo-zkvm-cli` receives CLI command | script/proof files | prover options and owns this boundary: Parse commands | Use shared opcode metadata | Run prove/verify workflows. It emits assembled script | proof report | inspection output, which are consumed by zkVM users | L2 prover service | L1 verification adapter.
