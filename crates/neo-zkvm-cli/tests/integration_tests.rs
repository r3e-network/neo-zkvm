...
#[test]
fn test_deterministic_hash_family_syscalls() {
    // Empty ByteString argument for each hash family member.
    let cases = [
        (
    ...
    for (name, expected_hex) in cases {
        let mut script = syscall(name);
        script.push(OpCode::RET.byte());
        let output = execute(ProofInput {
            script,
            arguments: vec![StackItem::ByteString(vec![])],
            gas_limit: 1_000_000,
        });
        assert_eq!(output.state, 0, "{name} should halt");
        let Some(StackItem::ByteString(digest)) = output.result else {
            panic!(
                "{name}: expected ByteString result, got {:?}",
                output.result
            );
        };
        assert_eq!(hex::encode(digest), expected_hex, "{name} digest mismatch");
    }
}
