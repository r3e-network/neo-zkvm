...
fn test_add_positive() {
    ...
    vm.load_script(vec![0x15, 0x17, 0x9E, 0x40]); // 5 + 7 = 12
    vm.run();
...
fn test_add_negative() {
    let mut vm = NeoVM::new(1_000_000);
    vm.load_script(vec![0x15, 0x0F, 0x9E, 0x40]); // 5 + (-1) = 4
    vm.run();
...
fn test_sub() {
    let mut vm = NeoVM::new(1_000_000);
    vm.load_script(vec![0x1A, 0x13, 0x9F, 0x40]); // 10 - 3 = 7
    vm.run();
...
fn test_mul() {
    let mut vm = NeoVM::new(1_000_000);
    vm.load_script(vec![0x16, 0x17, 0xA0, 0x40]); // 6 * 7 = 42
    vm.run();
...
fn test_div() {
    let mut vm = NeoVM::new(1_000_000);
    vm.load_script(vec![0x1F, 0x15, 0xA1, 0x40]); // 15 / 5 = 3
    vm.run();
...
fn test_mod() {
    let mut vm = NeoVM::new(1_000_000);
    vm.load_script(vec![0x1A, 0x13, 0xA2, 0x40]); // 10 % 3 = 1
    vm.run();
...
fn test_lt_true() {
    let mut vm = NeoVM::new(1_000_000);
    vm.load_script(vec![0x13, 0x15, 0xB5, 0x40]); // 3 < 5 = true
    vm.run();
...
fn test_lt_false() {
    let mut vm = NeoVM::new(1_000_000);
    vm.load_script(vec![0x15, 0x13, 0xB5, 0x40]); // 5 < 3 = false
    vm.run();
...
fn test_gt() {
    let mut vm = NeoVM::new(1_000_000);
    vm.load_script(vec![0x17, 0x13, 0xB7, 0x40]); // 7 > 3 = true
    vm.run();
    assert_eq!(vm.eval_stack.pop(), Some(StackItem::Boolean(true)));
}
...
fn test_dup() {
    let mut vm = NeoVM::new(1_000_000);
    vm.load_script(vec![0x15, 0x4A, 0x40]); // 5, DUP
    vm.run();
...
fn test_drop() {
    let mut vm = NeoVM::new(1_000_000);
    vm.load_script(vec![0x11, 0x12, 0x45, 0x40]); // 1, 2, DROP
    vm.run();
...
fn test_depth() {
    let mut vm = NeoVM::new(1_000_000);
    vm.load_script(vec![0x11, 0x12, 0x13, 0x43, 0x40]); // 1,2,3,DEPTH
    vm.run();
    assert_eq!(vm.eval_stack.pop(), Some(StackItem::Integer(3)));
}
...
fn test_assert_pass() {
    let mut vm = NeoVM::new(1_000_000);
    vm.load_script(vec![0x11, 0x39, 0x15, 0x40]); // PUSH1, ASSERT, PUSH5, RET
    vm.run();
    assert!(matches!(vm.state, VMState::Halt));
    assert_eq!(vm.eval_stack.pop(), Some(StackItem::Integer(5)));
}
...
fn test_assert_fail() {
    let mut vm = NeoVM::new(1_000_000);
    vm.load_script(vec![0x10, 0x39, 0x15, 0x40]); // PUSH0, ASSERT, PUSH5, RET
    vm.run();
    assert!(matches!(vm.state, VMState::Fault));
}
...
fn test_and() {
    let mut vm = NeoVM::new(1_000_000);
    vm.load_script(vec![0x1F, 0x17, 0x91, 0x40]); // 15 & 7 = 7
    vm.run();
    assert_eq!(vm.eval_stack.pop(), Some(StackItem::Integer(7)));
}
...
fn test_or() {
    let mut vm = NeoVM::new(1_000_000);
    vm.load_script(vec![0x18, 0x13, 0x92, 0x40]); // 8 | 3 = 11
    vm.run();
...
fn test_xor() {
    let mut vm = NeoVM::new(1_000_000);
    vm.load_script(vec![0x1F, 0x19, 0x93, 0x40]); // 15 ^ 9 = 6
    vm.run();
...
fn test_shl() {
    let mut vm = NeoVM::new(1_000_000);
    vm.load_script(vec![0x11, 0x13, 0xA8, 0x40]); // 1 << 3 = 8
    vm.run();
...
fn test_shr() {
    let mut vm = NeoVM::new(1_000_000);
    vm.load_script(vec![0x20, 0x12, 0xA9, 0x40]); // 16 >> 2 = 4
    vm.run();
    assert_eq!(vm.eval_stack.pop(), Some(StackItem::Integer(4)));
}
...
fn test_newarray() {
    let mut vm = NeoVM::new(1_000_000);
    vm.load_script(vec![0x13, 0xC3, 0x40]); // PUSH3, NEWARRAY
    vm.run();
...
fn test_pack_unpack() {
    ...
    vm.load_script(vec![0x11, 0x12, 0x13, 0x13, 0xC0, 0xC1, 0x40]);
    vm.run();
...
fn test_gas_limit() {
    ...
    vm.load_script(vec![0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x40]);
    vm.run();
    assert!(matches!(vm.state, VMState::Fault));
}
...
fn test_sufficient_gas() {
    ...
    vm.load_script(vec![0x11, 0x12, 0x9E, 0x40]); // 1+2
    vm.run();
    assert!(matches!(vm.state, VMState::Halt));
}
