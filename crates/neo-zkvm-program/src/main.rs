...
impl NeoVM {
    ...
    fn execute_opcode(&mut self, opcode: u8) -> bool {
        match opcode {
            ...
            0x0B => { self.push(StackItem::Null); true }
            0x0C => self.op_pushdata1(),
            0x0D => self.op_pushdata2(),
            0x0E => self.op_pushdata4(),
            0x0F => { self.push(StackItem::Integer(-1)); true }
            0x10..=0x20 => { self.push(StackItem::Integer((opcode - 0x10) as i128)); true }
            ...
            0x34 => self.op_call(),
            0x35 => self.op_call_l(),
            0x38 => { self.state = VMState::Fault; false } // ABORT
            0x39 => self.op_assert(),
            0x40 => self.op_ret(),
            
            // === Stack Operations ===
            0x43 => { self.push(StackItem::Integer(self.eval_stack.len() as i128)); true }
            0x45 => { self.pop(); true } // DROP
            0x46 => self.op_nip(),
            0x48 => self.op_xdrop(),
            0x49 => { self.eval_stack.clear(); true } // CLEAR
            0x4A => self.op_dup(),
            0x4B => self.op_over(),
            0x4D => self.op_pick(),
            0x4E => self.op_tuck(),
            ...
            0xC0 => self.op_pack(),
            0xC1 => self.op_unpack(),
            0xC2 => { self.push(StackItem::Array(vec![])); true }
            0xC3 => self.op_newarray(),
            0xC5 => { self.push(StackItem::Struct(vec![])); true }
            0xC6 => self.op_newstruct(),
            0xC8 => { self.push(StackItem::Map(vec![])); true }
            0xCA => self.op_size(),
            0xCB => self.op_haskey(),
    ...
    fn op_jmpeq(&mut self) -> bool {
        ...
        if a == b { self.jump(offset - 2) } else { true }
    }
    ...
    fn op_jmpeq_l(&mut self) -> bool {
        ...
        let b = self.pop().and_then(|i| i.to_integer()).unwrap_or(0);
        let a = self.pop().and_then(|i| i.to_integer()).unwrap_or(0);
        if a == b { self.jump(offset - 5) } else { true }
    }
    ...
    fn op_call(&mut self) -> bool {
        ...
        if let Some(ctx) = self.current_context() {
            ...
            self.invocation_stack.push(ExecutionContext { script, ip: target });
            true
        } else { false }
    }
    ...
    fn op_stloc_n(&mut self, n: u8) -> bool {
        if let Some(item) = self.pop() {
            let n = n as usize;
            if n < self.local_slots.len() { self.local_slots[n] = item; true }
            else { false }
        } else { false }
    }
    ...
    fn op_starg_n(&mut self, n: u8) -> bool {
        if let Some(item) = self.pop() {
            let n = n as usize;
            if n < self.argument_slots.len() { self.argument_slots[n] = item; true }
            else { false }
        } else { false }
    }
    ...
    fn op_add(&mut self) -> bool {
        let b = self.pop().and_then(|i| i.to_integer()).unwrap_or(0);
        let a = self.pop().and_then(|i| i.to_integer()).unwrap_or(0);
        self.push(StackItem::Integer(a.wrapping_add(b))); true
    }

    fn op_sub(&mut self) -> bool {
        let b = self.pop().and_then(|i| i.to_integer()).unwrap_or(0);
        let a = self.pop().and_then(|i| i.to_integer()).unwrap_or(0);
        self.push(StackItem::Integer(a.wrapping_sub(b))); true
    }

    fn op_mul(&mut self) -> bool {
        let b = self.pop().and_then(|i| i.to_integer()).unwrap_or(0);
        let a = self.pop().and_then(|i| i.to_integer()).unwrap_or(0);
        self.push(StackItem::Integer(a.wrapping_mul(b))); true
    }

    fn op_div(&mut self) -> bool {
        let b = self.pop().and_then(|i| i.to_integer()).unwrap_or(0);
        let a = self.pop().and_then(|i| i.to_integer()).unwrap_or(0);
        if b == 0 { self.state = VMState::Fault; false }
        else { self.push(StackItem::Integer(a / b)); true }
    }
    ...
    fn op_pow(&mut self) -> bool {
        ...
        self.push(StackItem::Integer(result)); true
    }

    fn op_sqrt(&mut self) -> bool {
        let a = self.pop().and_then(|i| i.to_integer()).unwrap_or(0);
        if a < 0 { self.state = VMState::Fault; false }
        else { self.push(StackItem::Integer((a as f64).sqrt() as i128)); true }
    }

    fn op_sign(&mut self) -> bool {
        let a = self.pop().and_then(|i| i.to_integer()).unwrap_or(0);
        self.push(StackItem::Integer(a.signum())); true
    }

    fn op_abs(&mut self) -> bool {
        let a = self.pop().and_then(|i| i.to_integer()).unwrap_or(0);
        self.push(StackItem::Integer(a.abs())); true
    }

    fn op_negate(&mut self) -> bool {
        let a = self.pop().and_then(|i| i.to_integer()).unwrap_or(0);
        self.push(StackItem::Integer(-a)); true
    }

    fn op_inc(&mut self) -> bool {
        let a = self.pop().and_then(|i| i.to_integer()).unwrap_or(0);
        self.push(StackItem::Integer(a + 1)); true
    }
    ...
    fn op_and(&mut self) -> bool {
        let b = self.pop().and_then(|i| i.to_integer()).unwrap_or(0);
        let a = self.pop().and_then(|i| i.to_integer()).unwrap_or(0);
        self.push(StackItem::Integer(a & b)); true
    }
    ...
    fn op_shl(&mut self) -> bool {
        ...
        let a = self.pop().and_then(|i| i.to_integer()).unwrap_or(0);
        self.push(StackItem::Integer(a << n)); true
    }
    ...
    fn op_nz(&mut self) -> bool {
        let a = self.pop().and_then(|i| i.to_integer()).unwrap_or(0);
        self.push(StackItem::Boolean(a != 0)); true
    }
    ...
    fn op_min(&mut self) -> bool {
        let b = self.pop().and_then(|i| i.to_integer()).unwrap_or(0);
        let a = self.pop().and_then(|i| i.to_integer()).unwrap_or(0);
        self.push(StackItem::Integer(a.min(b))); true
    }

    fn op_max(&mut self) -> bool {
        let b = self.pop().and_then(|i| i.to_integer()).unwrap_or(0);
        let a = self.pop().and_then(|i| i.to_integer()).unwrap_or(0);
        self.push(StackItem::Integer(a.max(b))); true
    }
    ...
    fn op_cat(&mut self) -> bool {
        ...
        a.extend(b);
        self.push(StackItem::ByteString(a)); true
    }
    ...
    fn op_substr(&mut self) -> bool {
        ...
        self.push(StackItem::ByteString(s[index..end].to_vec())); true
    }

    fn op_left(&mut self) -> bool {
        let count = self.pop().and_then(|i| i.to_integer()).unwrap_or(0) as usize;
        let s = self.pop().map(|i| i.to_bytes()).unwrap_or_default();
        self.push(StackItem::ByteString(s[..count.min(s.len())].to_vec())); true
    }
    ...
    fn op_right(&mut self) -> bool {
        ...
        self.push(StackItem::ByteString(s[start..].to_vec())); true
    }
    ...
    fn op_pack(&mut self) -> bool {
        ...
        for _ in 0..n {
            if let Some(item) = self.pop() { items.push(item); }
        }
        items.reverse();
        self.push(StackItem::Array(items)); true
    }
    ...
    fn op_newarray(&mut self) -> bool {
        let n = self.pop().and_then(|i| i.to_integer()).unwrap_or(0) as usize;
        self.push(StackItem::Array(vec![StackItem::Null; n])); true
    }

    fn op_newstruct(&mut self) -> bool {
        let n = self.pop().and_then(|i| i.to_integer()).unwrap_or(0) as usize;
        self.push(StackItem::Struct(vec![StackItem::Null; n])); true
    }
    ...
    fn op_size(&mut self) -> bool {
        ...
        self.push(StackItem::Integer(size as i128)); true
    }
    ...
    fn op_haskey(&mut self) -> bool {
        ...
        let has = match (container, key) {
            ...
            (Some(StackItem::Map(m)), Some(k)) => 
                m.iter().any(|(mk, _)| *mk == k),
            _ => false,
        };
        self.push(StackItem::Boolean(has)); true
    }
    ...
    fn op_append(&mut self) -> bool {
        ...
        if let (Some(StackItem::Array(mut a)), Some(i)) = (self.pop(), item) {
            a.push(i);
            self.push(StackItem::Array(a)); true
        } else { false }
    }
    ...
    fn op_setitem(&mut self) -> bool {
        ...
        match (container, key, value) {
            (Some(StackItem::Array(mut a)), Some(StackItem::Integer(i)), Some(v)) => {
                let idx = i as usize;
                if idx < a.len() { a[idx] = v; self.push(StackItem::Array(a)); true }
                else { false }
            }
            _ => false,
        }
    ...
    fn op_remove(&mut self) -> bool {
        ...
        match (container, key) {
            (Some(StackItem::Array(mut a)), Some(StackItem::Integer(i))) => {
                let idx = i as usize;
                if idx < a.len() { a.remove(idx); self.push(StackItem::Array(a)); true }
                else { false }
            }
            _ => false,
        }
    ...
    fn op_clearitems(&mut self) -> bool {
        match self.pop() {
            Some(StackItem::Array(_)) => { self.push(StackItem::Array(vec![])); true }
            Some(StackItem::Map(_)) => { self.push(StackItem::Map(vec![])); true }
            _ => false,
        }
    }
    ...
    fn op_isnull(&mut self) -> bool {
        ...
        self.push(StackItem::Boolean(is_null)); true
    }

    fn op_istype(&mut self) -> bool {
        let type_id = self.read_byte().unwrap_or(0);
        let item = self.pop();
        let matches = match (item, type_id) {
            (Some(StackItem::Boolean(_)), 0x20) => true,
            (Some(StackItem::Integer(_)), 0x21) => true,
            (Some(StackItem::ByteString(_)), 0x28) => true,
            (Some(StackItem::Array(_)), 0x40) => true,
            (Some(StackItem::Struct(_)), 0x41) => true,
            (Some(StackItem::Map(_)), 0x48) => true,
            _ => false,
        };
        self.push(StackItem::Boolean(matches)); true
    }
}
