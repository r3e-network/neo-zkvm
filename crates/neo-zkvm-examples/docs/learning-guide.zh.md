# neo-zkvm-examples 技术学习指南

这份指南把 `neo-zkvm-examples` 当作 Neo N4 的一个技术单元来解释。它不是源码阅读图，而是帮助读者理解：这个单元负责什么、哪些技术假设保证它正确、数据如何移动、状态如何变化、证据如何被验证、它如何接入 Neo N4 的整体架构。
...
| 输入 | 示例脚本 <br> 示例输入 <br> 本地 prover |
| 职责 | 演示 API <br> 覆盖边界案例 <br> 记录期望输出 |
| 输出 | 示例证明 <br> 教程输出 <br> 回归样本 |
...
`neo-zkvm-examples` 接收 示例脚本 | 示例输入 | 本地 prover，拥有的边界是：演示 API | 覆盖边界案例 | 记录期望输出。它输出 示例证明 | 教程输出 | 回归样本，然后由 zkVM 用户 | L2 prover 服务 | L1 验证适配器 消费。
