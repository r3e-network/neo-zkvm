...
impl NeoProver {
    ...
    pub fn prove(&self, input: ProofInput) -> NeoProof {
        ...
        let (proof_bytes, vkey_hash, actual_mode) = match self.config.proof_mode {
            ProofMode::Execute => (vec![], [0u8; 32], ProofMode::Execute),
            ProofMode::Mock => (
            ...
            ProofMode::Plonk if sp1_available => {
                match self.generate_sp1_proof(&input, SP1ProofMode::Plonk) {
                    Ok((bytes, hash)) => (bytes, hash, ProofMode::Plonk),
                    Err(_) => {
            ...
            ProofMode::Groth16 if sp1_available => {
                match self.generate_sp1_proof(&input, SP1ProofMode::Groth16) {
                    Ok((bytes, hash)) => (bytes, hash, ProofMode::Groth16),
                    Err(_) => {
