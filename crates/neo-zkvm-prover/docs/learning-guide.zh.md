# neo-zkvm-prover 技术学习指南

这份指南把 `neo-zkvm-prover` 当作 Neo N4 的一个技术单元来解释。它不是源码阅读图，而是帮助读者理解：这个单元负责什么、哪些技术假设保证它正确、数据如何移动、状态如何变化、证据如何被验证、它如何接入 Neo N4 的整体架构。
...
| 输入 | 证明输入 <br> guest 程序 <br> prover 模式 |
| 职责 | 哈希输入 <br> 运行 guest 执行 <br> 构建证明封装 |
| 输出 | NeoProof <br> 公开输出 <br> prover 报告 |
...
`neo-zkvm-prover` 接收 证明输入 | guest 程序 | prover 模式，拥有的边界是：哈希输入 | 运行 guest 执行 | 构建证明封装。它输出 NeoProof | 公开输出 | prover 报告，然后由 zkVM 用户 | L2 prover 服务 | L1 验证适配器 消费。
